...
**基于[MorseFold](https://github.com/qingchenyouforcc/MorseFold)的AI迭代测试仓库**

一种面向摩斯码序列的可逆轻量压缩实验：先将文本映射为标准摩斯码，再基于“连续同长度分组 + 规则交替结构 + 紧凑元信息 token + 消息内重复词回指”生成更短的简化表示，并支持完整解码回原始文本。

## Abstract

本项目研究一个很具体的问题：标准摩斯码本身已经是离散符号序列，但在字符级仍存在明显结构冗余，例如相同长度的连续模式、点划比例偏向、严格交替的规则结构，以及同一条消息内的重复单词。基于这些特征，项目提出了一种可逆简化编码方法。该方法不追求全局最优压缩，而是通过局部规则压缩、紧凑 token 化和消息内回指，并结合“收益不足即回退”的策略，在保证解码稳定性的前提下尽量缩短编码长度。
...
核心实现位于 [`encoder.py`](encoder.py) 的 `text_to_morseSimplify()`，当前整体流程如下：

1. 使用 [`main.py`](main.py) 中的 `text_to_morse()` 将文本转为标准摩斯码
2. 在每个单词内部，按“连续且长度相同”的摩斯码序列分组
3. 对同长度 run 执行最短表示搜索，允许“原样输出/压缩输出”混合分段
4. 将压缩段中的每个摩斯码转换为位置标识 `ID`
5. 将组级规则符号 `RS` 编码为单字符 token
6. 若压缩后不比原始组更短，则回退为原始摩斯组
7. 每个单词完成简化后，若该简化词在同一条消息中已出现，且 `~<index>` 更短，则改为回指输出
8. 输出可逆的简化编码串
...
每个候选压缩段在逻辑上都会生成一个 `RS`，格式仍为 `<length><tail>`，例如 `4-`、`3.`。
...
- 编码器会直接比较 `tail = -` 和 `tail = .` 两个方向的真实输出长度，保留更短结果

在当前协议里，这个逻辑 `RS` 会进一步压成单字符 token，例如：

- `G -> 4-`
- `H -> 4.`
- `J -> 5.`

[`decoder.py`](decoder.py) 仍兼容早期 `%RS` 形式。
...
- 位置从 `1` 开始编号
- 多个位置直接拼接成字符串
- 在当前协议里，高频 `ID` 文本还会进一步映射成单字符 token
...
该格式由 [`decoder.py`](decoder.py) 完整解码回标准摩斯码与普通文本，并继续兼容早期 `%RS` 和反斜杠分隔格式。
...
- 某个简化词第一次出现时正常输出
- 同一条消息中再次出现时，可替换为 `~<index>`
- `index` 使用 `0-9a-zA-Z`
- 只有当回指形式更短时才会启用

这是一种基于消息局部上下文的可逆回指机制，不是依赖 benchmark 词频的静态词典。
...
- 两个字符长度都为 `4`，因此可分为同一组
- 逻辑组偏向结果为 `4-`，当前 token 为 `G`
- `--.-` 中 `.` 在第 `3` 位，其 `ID` 映射为 token `f`
- `-.-.` 为规则交替码，其 `ID` 为 `+`，映射为 token `b`
...
这个例子展示了当前版本的“重复词回指”：第二个 `HELLO` 不再重复完整展开，而是直接引用本条消息中编号为 `0` 的第一个简化词。
...
以下结果来自按当前实现重新生成的实验输出 [`output/experiment_group_summary.csv`](output/experiment_group_summary.csv)。
...
- 单词级和结构化短样本上，紧凑 `ID`/`RS` token 带来的收益最明显
- 多词和长文本场景下仍保持清晰正收益，说明当前格式并不只适用于玩具示例
- 同一条消息中的重复词现在还能通过 `~<index>` 继续复用，当前编码器和解码器都已支持
...
- 压缩主轴仍然是局部分组加消息内重复词复用，不是全局最优解析
- 当前字符集主要面向英文、数字和常见英文标点
- 对结构不规律、重复较少的摩斯序列，收益可能有限
- `ID` 已做紧凑化，但更长码长场景下仍可能残留元信息开销
...
- 设计更强的跨段复用机制，而不只是组内压缩和重复词回指
- 研究更通用的消息级模板复用，而不是依赖数据集特调词典
- 为低频 `ID` 模式设计更紧凑的补充编码方案
- 引入更系统的实验对照，如与 RLE、Huffman 风格方案比较
- 增加论文式评估指标，如熵估计、解码复杂度、吞吐表现

## Optimization Notes

当前仓库状态已经包含 [`LOG.md`](LOG.md) 中记录的六轮优化：

- 第一次优化：同长度 run 内最短分段搜索
- 第二次优化：缓存化和增量式 DP 构造
- 第三次优化：长度 `1` 和 `2` 的短 run 也纳入最短表示搜索
- 第四次优化：`RS` 单字符 token
- 第五次优化：`ID` 单字符紧凑 token
- 第六次优化：消息内重复词 `~<index>` 回指
